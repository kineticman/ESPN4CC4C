...
def write_programmes(f, rows):
    ...
    for r in rows:
        ...
        is_reair = safe_get(r, "is_reair", 0)  # 0 = not a re-air, 1 = is a re-air
        ...
        if kind == "event":
            ...
            # NOTE: Do NOT add a generic 'Live' category here.
            # Live vs Replay is handled below using event_type + is_reair.
            ...
            # Event type / re-air categories:
            # - For re-airs, tag explicitly as Replay
            # - For true live events, rely on <live>1</live> instead of another "Live" category
            et = (event_type or "").upper()
            if is_reair == 1 or et == "REPLAY":
                # Normalize to a single 'Replay' label
                f.write("    <category>Replay</category>\n")
            ...
            # LIVE tag - only for live events that are NOT re-airs
            if event_type and event_type.upper() == "LIVE" and is_reair != 1:
                f.write("    <live>1</live>\n")
