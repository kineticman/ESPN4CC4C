...
def parse_args():
    ap = argparse.ArgumentParser(description="ESPN Clean v2.0 Plan Builder (sticky lanes)")
    ...
    ap.add_argument("--stick-window-hours", type=int, default=12,
...
def make_default_lanes(n:int=40, start_chno:int=20010):
    return [(f"eplus{i}", start_chno + (i-1), f"ESPN+ EPlus {i}", "ESPN+ VC") for i in range(1, n+1)]
...
def latest_two_plan_ids(conn):
    rows = conn.execute("SELECT DISTINCT plan_id FROM plan_slot ORDER BY plan_id DESC LIMIT 2").fetchall()
...
def build_plan_with_stickiness(conn, channels, events, start_dt_utc, end_dt_utc, min_gap,
                               priormap:dict, stick_window:timedelta):
    ...
    for e in events:
        ...
        if t <= start_dt_utc or s >= end_dt_utc:
            continue
        s = max(s, start_dt_utc); t = min(t, end_dt_utc)
        if t <= s: continue
...
def main():
    ...
    events = load_events(conn, start_utc.replace(tzinfo=timezone.utc).isoformat(),
    ...
    jlog(event="plan_build_start", version=VERSION, db=args.db, valid_hours=args.valid_hours,
         ...
         sticky_events=len(pmap), stick_window_hours=args.stick_window_hours)

    plan_slots = build_plan_with_stickiness(conn, channels, events, start_utc, end_utc,
                                            min_gap, pmap, stick_window)
