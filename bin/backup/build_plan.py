...
try:
    ...
    from config import BUILDER_DEFAULT_TZ as CFG_TZ
...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=int,
        default=30,
        help="grid alignment in minutes (e.g., 30 for :00/:30)",
    )
    ap.add_argument(
        "--lanes",
        type=int,
    ...
    for e in norm_events:
        ...
        if chosen is None:
            ...
            if best_cid is None:
                ...
                cid = min(
                    channels_sorted,
                    key=lambda k: timelines[k][-1][1] if timelines[k] else start_dt_utc,
                )
            ...
            snapped.append(
                {
                    ...
                    "placeholder_reason": (
                        s["placeholder_reason"] if s["kind"] != "event" else None
                    ),
                }
            )
