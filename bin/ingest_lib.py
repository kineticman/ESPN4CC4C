...
class RetryHTTP:
    ...
    def get_json(self, url:str, headers:Dict[str,str]=None, params:Dict[str,str]=None)->Any:
        last = None
        for i in range(1, self.max_tries+1):
            try:
                r = requests.get(url, headers=headers or {}, params=params or {}, timeout=self.timeout)
                r.raise_for_status()
