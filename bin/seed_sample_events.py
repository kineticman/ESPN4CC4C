#!/usr/bin/env python3
import sqlite3, uuid, argparse
from datetime import datetime, timedelta, timezone

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--db", default="data/eplus_vc.sqlite3")
    ap.add_argument("--channels", type=int, default=4, help="how many lanes to seed starting from eplus1")
    ap.add_argument("--start-offset", type=int, default=5, help="minutes from now to start")
    ...
    with conn:
        for i in range(1, args.channels+1):
            ...
            sport = "Demo"
            conn.execute("INSERT OR IGNORE INTO events(id,start_utc,stop_utc,title,sport,subtitle,summary,image) VALUES(?,?,?,?,?,?,?,?)",
                         (eid, start.isoformat(), stop.isoformat(), title, sport, "Seeded", "Sample seeded event", None))
            # dummy feed per lane (replace with real later)
            feed = f"http://example.com/stream/eplus{i}.m3u8"
            conn.execute("INSERT INTO feeds(event_id,url,is_primary) VALUES(?,?,1)", (eid, feed))
