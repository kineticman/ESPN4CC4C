...
def jlog(**kv):
    kv = {"ts": datetime.now(timezone.utc).isoformat(), "mod":"build_plan_sticky2", **kv}
...
def build_plan_sticky(
    ...
    timelines: Dict[str, List[Tuple[datetime, datetime, Optional[str], str]]] = {lid: [] for lid in lane_ids}
    ...
    def affinity_order(center: str) -> List[str]:
        ...
        for d in range(1, lane_affinity+1):
            left = ci - d
            right = ci + d
            if left >= 0:  order.append(lane_ids[left])
            if right < len(lane_ids): order.append(lane_ids[right])
        return order
    ...
    for lid in lane_ids:
        ...
        for (s, t, eid, kind) in timelines[lid]:
            if s - cursor >= min_gap:
                plan_slots.append({
                    "channel_id": lid, "event_id": None, "start": cursor, "end": s,
                    "kind": "placeholder", "placeholder_reason": "no_event"
                })
            plan_slots.append({
                "channel_id": lid, "event_id": eid, "start": s, "end": t,
                "kind": "event", "placeholder_reason": None
            })
            cursor = t
        if end_dt_utc - cursor >= min_gap:
            plan_slots.append({
                "channel_id": lid, "event_id": None, "start": cursor, "end": end_dt_utc,
...
def parse_args():
    ...
    ap.add_argument("--stick-window-hours", type=int, default=24, help="prefer previous lane within this horizon")
    ap.add_argument("--freeze-hours", type=int, default=3, help="never spill (no truncation) within this horizon")
    ap.add_argument("--lane-affinity", type=int, default=2, help="try ±N neighbor lanes around previous lane")
...
def main():
    ...
    jlog(event="plan_build_start", version=VERSION, db=args.db,
         valid_hours=args.valid_hours, tz=args.tz,
         start_utc=start_utc.isoformat(), end_utc=end_utc.isoformat(),
         channels=len(channels), events=len(events),
         seeded_channels=seeded, stick_window_h=args.stick_window_hours,
         freeze_h=args.freeze_hours, lane_affinity=args.lane_affinity,
         prev_event_map=len(prev_map))

    plan_slots = build_plan_sticky(
        channels, events, start_utc, end_utc, min_gap,
