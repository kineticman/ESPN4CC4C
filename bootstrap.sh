...
# Build a single-line base64 payload (portable: no -w flag)
cron_payload="$(cat <<'CRONFILE'
SHELL=/bin/bash
...
cron_b64="$(printf "%s" "$cron_payload" | base64 | tr -d '\n')"

docker compose exec -T espn4cc sh -lc "echo $cron_b64 | base64 -d > /etc/cron.d/espn4cc && chmod 644 /etc/cron.d/espn4cc && mkdir -p /app/logs && : > /app/logs/cron.log && chmod 666 /app/logs/cron.log && pkill -HUP cron || true"
