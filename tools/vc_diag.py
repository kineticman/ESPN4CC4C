...
def probe_resolver(base: str, lane: str) -> Dict[str, object]:
    ...
    try:
        r["whatson"] = json.loads(body) if code == 200 else None
    except Exception:
        r["whatson"] = None
...
def db_programme_counts(conn: sqlite3.Connection):
    ...
    pid = int(row["pid"])
    total = conn.execute(
        "SELECT COUNT(*) FROM plan_slot WHERE plan_id=?", (pid,)
    ).fetchone()[0]
    ev = conn.execute(
        "SELECT COUNT(*) FROM plan_slot WHERE plan_id=? AND kind='event'", (pid,)
    ).fetchone()[0]
    ph = conn.execute(
        "SELECT COUNT(*) FROM plan_slot WHERE plan_id=? AND kind='placeholder'", (pid,)
    ).fetchone()[0]
...
def db_channel_count(conn: sqlite3.Connection) -> Optional[int]:
    try:
        return conn.execute(
            "SELECT COUNT(DISTINCT id) FROM channel WHERE active=1"
        ).fetchone()[0]
    except Exception:
        return None
...
def xml_now_for_channel(
    xml_path: str, chan_id: int, ts_utc: Optional[datetime] = None
) -> List[Dict[str, str]]:
...
def main():
    ...
    ap.add_argument(
        ...
        default=os.getenv(
            ...
            env.get(
                ...
                env.get("RESOLVER_BASE", "http://127.0.0.1:8094"),
            ),
        ),
    )
    ap.add_argument(
        "--xml", default=xml_path, help="path to epg.xml (defaults to OUT in .env)"
    )
    ap.add_argument("--since-mins", type=int, default=30)
    ap.add_argument(
        "--quiet-errors",
        action="store_true",
        help="only show a count of recent resolver errors",
    )
    ap.add_argument(
        "--no-fleet", action="store_true", help="skip sweeping all channels"
    )
    ap.add_argument(
        "--env-file", default=default_env, help="path to .env.plan (auto-loaded)"
    )
    ap.add_argument(
        "--audit", action="store_true", help="run overlap/mismatch audits for --lane"
    )
    args = ap.parse_args()
    ...
    p(
        "when/where",
        f"now(utc)={now_utc_iso()}\nresolver={args.resolver}\ndb={args.db}\nxml={args.xml}\nlane={args.lane}\nenv_file={args.env_file if os.path.isfile(args.env_file) else '(none)'}",  # noqa: E501
    )
    ...
    p(
        "resolver probes",
        f"/health={rp['health']}  /epg.xml={rp['epg']}  /playlist.m3u={rp['m3u']}  /channels={rp['channels_code']}/{rp['channels_len']}  lane_debug_ok={rp['lane_debug_ok']}",  # noqa: E501
    )
    if rp["routes"]:
    ...
    if db_ok:
        ...
        if isinstance(lane_id_num, int):
            p("lane→channel_id", str(lane_id_num))
        slot, feeds = db_slot_and_feeds(conn, args.lane, ts)
        p(
            f"db: current slot for {args.lane}",
            json.dumps(dict(slot) if slot else {}, indent=2),
        )
        if slot_val(slot, "event_id") is not None:
            p(
                "db: feeds for current event",
        ...
        preview = "\n".join(
            f"- {r['channel_id']} {r['start_utc']}→{r['end_utc']} {r['placeholder_reason']}"
            for r in rows[:12]
        )
        ...
        p(
            "db: programme counts (latest plan_id)",
            f"total={total_slots}  events={ev}  placeholders={ph}",
        )
    ...
    p(
        "xml: programme count",
        f"{xml_prog if xml_prog is not None else '(missing or unreadable)'}",
    )
    p(
        "xml: channel count",
        f"{xml_chan if xml_chan is not None else '(missing or unreadable)'}",
    )
    if gen_tag:
        p("xml: generator", gen_tag)
    if db_ok:
        ...
        if ch_id is not None:
            ...
            if xml_now:
                p(
                    "xml: NOW for lane",
                    "\n".join(
                        [f"- {x['start']} → {x['stop']}  {x['title']}" for x in xml_now]
                    ),
                )
            else:
            ...
            if dov:
                ...
                lines = [
                    f"- {r['a_start']}→{r['a_end']} {r['a_kind']} vs {r['b_start']}→{r['b_end']} {r['b_kind']}"
                    for r in dov[:8]
                ]
            ...
            if len(xml_now) > 1:
                audit_warns.append(
                    f"XML has multiple 'now' items for lane {ch_id} (overlap)"
                )
    ...
    if rp["health"] == 200:
        ok_items.append("Resolver /health 200")
    elif rp["health"] in (0, None):
        bad_items.append("Resolver not reachable")
    else:
        warn_items.append(f"Resolver /health HTTP {rp['health']}")

    ok_items.append("/epg.xml 200" if rp["epg"] == 200 else "/epg.xml not OK")
    if rp["m3u"] == 200:
        ok_items.append("/playlist.m3u 200")
    else:
        warn_items.append("/playlist.m3u not OK")

    if rp.get("channels_code") == 200:
        if isinstance(rp["channels_len"], int):
            if rp["channels_len"] > 0:
                ok_items.append(f"/channels 200 ({rp['channels_len']} entries)")
            else:
                warn_items.append("/channels 200 but empty array")
        else:
    ...
    if rp["lane_debug_ok"]:
        ok_items.append(f"/vc/{args.lane}/debug OK")
    else:
        warn_items.append(f"/vc/{args.lane}/debug unavailable")

    if db_ok:
        total_slots = db_counts[1]
        if pid is not None:
            ok_items.append(f"plan_id={pid}")
        else:
            bad_items.append("No plan_id in plan_slot")

        if n_off == 0:
            ok_items.append("no off-minute placeholders")
        else:
            warn_items.append(f"{n_off} off-minute placeholders")
        ...
        if total_slots is not None and xml_prog is not None:
            if total_slots == xml_prog:
                ...
                warn_items.append(
                    f"programmes mismatch (DB={total_slots} XML={xml_prog})"
                )
        ...
        if not args.no_fleet:
            ok, missing, ph = fleet
            info_items.append(
                f"fleet: ok={ok} missing_feed={missing} placeholders={ph}"
            )

        # fold audits
        for w in audit_warns:
            warn_items.append(w)
        for i in audit_infos:
            info_items.append(i)
    ...
    if rp["epg"] != 200:
        suggestions.append(
            "Resolver /epg.xml failing → check vc-resolver-v2 logs & app module path."
        )
    if db_ok and pid is None:
        suggestions.append("No plan rows → run vc-plan.service to build plan.")
    if db_ok and n_off > 0:
        suggestions.append(
            "Off-minute placeholders → rebuild with ALIGN=30 and clamp enabled."
        )
    if (
        db_ok
        and slot_val(slot, "kind") == "event"
        and (feeds is not None and len(feeds) == 0)
    ):
        suggestions.append(
            "Event has no feed → re-run ingest or verify ESPN watch IDs."
        )
    if rp["health"] in (0, None):
        suggestions.append(
            "Resolver unreachable → verify service is active and bound to :8094."
        )
    ...
    if gen_tag and "espn-clean-v2.1" not in gen_tag:
        suggestions.append(
            "XML generator old → regenerate XML and refresh Channels DVR sources."
        )
    if args.audit and "NOW mismatch" in " ".join(warn_items):
        suggestions.append(
            "NOW mismatch → ensure XML writer exports latest plan only and dedup per (channel,start,stop)."
        )
    ...
    if slot is not None:
        print(
            f"\nCurrent slot: {slot['channel_id']} {slot['start_utc']}→{slot['end_utc']} {slot['kind']} (event_id={slot_val(slot,  'event_id',  '-')})"  # noqa: E501
        )
