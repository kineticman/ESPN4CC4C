#!/bin/bash
# ESPN4CC4C Deeplink Format Testing Script - FIXED VERSION
# Works with actual database schema (no event_id column)
...
ORDER BY 
    ...
    e.start_utc
LIMIT $LIMIT
" | while IFS='|' read -r network network_id title play_id simulcast_id event_type; do
    ...
    # Format 11: Try using airingId as if it were eventId
    echo "# Format 11: showEvent with airingId (may work as eventId) ⭐" >> "$OUTPUT_FILE"
    ...
    # Format 12: playEvent with airingId
    echo "# Format 12: playEvent with airingId" >> "$OUTPUT_FILE"
    ...
    # Format 13: showEvent with both IDs
    echo "# Format 13: showEvent + playID (hybrid)" >> "$OUTPUT_FILE"
    ...
    # Format 14: Alternative airing-based event
    echo "# Format 14: showAiring endpoint" >> "$OUTPUT_FILE"
...
2. Test each format systematically:
   - START WITH FORMAT 11 (showEvent with airingId) - most likely to work!
...
5. Look for patterns:
   ...
   - Does showEvent work with airingId?

========================================
MOST LIKELY TO WORK (PREDICTIONS)
========================================

⭐ Format 11: showEvent with airingId
  → BEST GUESS - may bypass channel MVPD auth
  → Uses airingId as eventId (they might be interchangeable)
...
Format 14: showAiring endpoint
  → Dedicated airing-specific endpoint
...
Event-based deeplinks (Format 11-14):
  ...
  → Target specific airing, not channel feed
...
ESPN2 (espn2):
  ...
  - Format 11, 2, or 4 likely fixes it
  
ACCN (acc):
  - Format 1 probably fails
  - Format 11, 2, 4, or 7 might work
...
6. Whether Format 11 (showEvent with airingId) bypasses MVPD auth
