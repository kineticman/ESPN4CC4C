...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -type=*|--type=*)
            ...
            ;;
        --live-now)
            # Note: Use different filters for main query (with alias) vs summary query (without alias)
            LIVE_NOW_FILTER="AND datetime('now') BETWEEN datetime(e.start_utc) AND datetime(e.stop_utc)"
            LIVE_NOW_FILTER_NO_ALIAS="AND datetime('now') BETWEEN datetime(start_utc) AND datetime(stop_utc)"
            EVENT_TYPE_FILTER="('LIVE')"
            shift
            ;;
        --help|-h)
